...
import template from "./model-popup.html";
...
import { TypeDefinition, TypeDefinitionProperty, TypeDefinitionPropertyTypeArrayOfPrimitive, TypeDefinitionPropertyTypeArrayOfReference, TypeDefinitionPropertyTypeCombination, TypeDefinitionPropertyTypePrimitive, TypeDefinitionPropertyTypeReference } from "../../../../../models/typeDefinition";
...
export class ModelPopup {
    ...
    constructor(
        ...
        this.responsename = ko.observable();
    ...
    @Param()
    public responsename: ko.Observable<any>;
    ...
    public getDefinitionForRepresentation(): TypeDefinition {
        var data=  this.operation().getMeaningfulResponses().find(x=> x.statusCode.code == this.responsename().statusCode.code)
        let definition = this.definitions().find(x => x.name === data.representations[0].typeName);

        if (!definition) {
            // Fallback for the case when type is referenced, but not defined in schema.
            return new TypeDefinition(data.representations[0].typeName, {}, this.definitions());
        }
        ...
        if (!definition.name) {
            definition.name = data.representations[0].typeName;
        }
        this.example= data.representations[0].examples[0];
        return definition;
    }
